...
export function calcStochRSISeries(
  ...
  const smoothedK: (number | null)[] = rawK.map((_, i) => {
    const window = rawK
      .slice(Math.max(0, i - smoothK + 1), i + 1)
  ...
  const smoothedD: (number | null)[] = smoothedK.map((_, i) => {
    const window = smoothedK
      .slice(Math.max(0, i - smoothD + 1), i + 1)
      .filter((v) => v !== null) as number[];
...
export function calcMACD(
  close: number,
  state: MACDState,
  fast: number = INDICATOR_PERIODS.MACD_FAST,
  slow: number = INDICATOR_PERIODS.MACD_SLOW,
  signal: number = INDICATOR_PERIODS.MACD_SIGNAL
...
export function calcBB(
  ...
  stdDevMult: number = INDICATOR_PERIODS.BOLLINGER_BANDS_STDDEV
): {
  upper: number | null;
  middle: number | null;
  lower: number | null;
...
export function calcADX(
  ...
  const plusDM =
    candle.h - state.prevHigh > state.prevLow! - candle.l
      ? Math.max(candle.h - state.prevHigh, 0)
      : 0;
  const minusDM =
    state.prevLow! - candle.l > candle.h - state.prevHigh
      ? Math.max(state.prevLow! - candle.l, 0)
      : 0;
  ...
  if (state.plusDM === null) {
    ...
    if (state.seedTR.length === period) {
      ...
      state.seedTR = [sumTR];
      state.seedPlus = [sumPlus];
      state.seedMinus = [sumMinus];
    }
    return { adx: null, plusDI: null, minusDI: null };
  }
...
export function calcPSAR(
  ...
  if (prevBull) {
    if (candle.l < sar) {
      ...
      if (candle.h > prevEP) {
        state.ep = candle.h;
        state.af = Math.min(prevAF + step, max);
      }
...
export function calcVWAP(
  ...
  if (state.cumVol === 0)
    return { vwap: null, upper1: null, lower1: null, upper2: null, lower2: null };
...
export function detectPatterns(candle: Candle, prev: Candle | null): PatternResult[] {
  ...
  if ((upperRatio > 0.6 || lowerRatio > 0.6) && bodyRatio < 0.2) {
    results.push({
      name: 'pin_bar',
      bull: lowerRatio > upperRatio,
      label: lowerRatio > upperRatio ? '📌↑' : '📌↓',
    });
  }
...
export function scoreEntryQuality(
  ...
  if (dir === 'long') {
    if (e9 > e20 && e20 > e50) {
      score += 30;
      factors.push('Full bullish stack');
    } else if (e9 > e50) {
      score += 15;
    ...
    if (e9 < e20 && e20 < e50) {
      score += 30;
      factors.push('Full bearish stack');
    } else if (e9 < e50) {
      score += 15;
  ...
  if (dir === 'long') {
    if (rsi > 50 && rsi < 65) {
      score += 25;
      factors.push('RSI momentum zone');
    } else if (rsi >= 40 && rsi <= 50) {
      score += 15;
      factors.push('RSI midzone');
    } else if (rsi < 35) {
      score += 20;
      factors.push('RSI oversold bounce');
    } else if (rsi >= 65) {
      score -= 10;
    ...
    if (rsi < 50 && rsi > 35) {
      ...
      score += 20;
      factors.push('RSI overbought fade');
    } else if (rsi <= 35) {
      score -= 10;
  ...
  if (dir === 'long' && price > e20) {
    score += 20;
    factors.push('Price > EMA20');
  }
  if (dir === 'short' && price < e20) {
    score += 20;
    factors.push('Price < EMA20');
  }
  ...
  if (recent) {
    ...
    if (recent.type === 'bull' && dir === 'long' && age < 30) {
      score += 25;
      factors.push('Fresh bull cross');
    }
    if (recent.type === 'bear' && dir === 'short' && age < 30) {
      score += 25;
      factors.push('Fresh bear cross');
    }
  }
  ...
  if (score >= 75) {
    ...
    label = '◆ GOOD SETUP';
    cls = dir === 'long' ? 'strong-long' : 'strong-short';
  } else if (score >= 30) {
    label = '◇ WEAK SETUP';
    cls = 'weak';
  } else {
