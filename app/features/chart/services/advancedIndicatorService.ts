...
export function calcVolumeProfile(candles: Candle[], numBuckets = 24): VolumeProfile {
  if (!candles.length) {
...
export function calcAutoFibo(candles: Candle[], lookback = 50): FiboOverlay | null {
  ...
  const levels: FiboLevel[] = FIBO_LEVELS.map((ratio, i) => {
    const price = dir === 'up' ? swingHigh - ratio * range : swingLow + ratio * range;
