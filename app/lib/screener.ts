...
function computeIndicators(candles: Candle[], nBar = 20) {
  ...
  let e9: number | null = null,
    e20: number | null = null,
    e50: number | null = null;
  let ef: number | null = null,
    es: number | null = null,
    esig: number | null = null;
  ...
  let prevEf: number | null = null,
    prevEs: number | null = null,
    prevSig: number | null = null;
  ...
  if (atr !== null && n > WILDER_PERIOD) {
    ...
    for (let i = 1; i < n; i++) {
      ...
      const newUpper: number =
        upperBand !== null && rawUpper < upperBand ? rawUpper : (upperBand ?? rawUpper);
      const newLower: number =
        lowerBand !== null && rawLower > lowerBand ? rawLower : (lowerBand ?? rawLower);
  ...
  try {
    ...
    if (fibo) {
      ...
      for (const lv of fibo.levels) {
        if (Math.abs(lv.price - last.c) / last.c < fibThreshold) {
          nearFib = lv.label;
          break;
        }
      }
...
function evaluateAllFilters(
  ...
  for (const f of ALL_FILTER_IDS) {
    ...
    switch (f) {
      case 'ema_stack_bull':
        fired = !!(e9 && e20 && e50 && e9 > e20 && e20 > e50);
        break;
      case 'ema_stack_bear':
        fired = !!(e9 && e20 && e50 && e9 < e20 && e20 < e50);
        break;
      case 'rsi_oversold':
        fired = rsi !== null && rsi < 30;
        break;
      case 'rsi_overbought':
        fired = rsi !== null && rsi > 70;
        break;
      ...
      case 'supertrend_bear':
        ...
        break;
      case 'bb_squeeze':
        fired = bbWidth !== null && bbWidth < bbThresh;
        break; // SIGNAL-3
      case 'volume_spike':
        fired = !!(volAvg20 && lastVol > volAvg20 * volMult);
        break; // SIGNAL-5
      case 'price_near_ema9':
        fired = !!(e9 && Math.abs(price - e9) / price < 0.003);
        break;
      case 'price_near_ema20':
        fired = !!(e20 && Math.abs(price - e20) / price < 0.003);
        break;
      case 'price_near_ema50':
        fired = !!(e50 && Math.abs(price - e50) / price < 0.005);
        break;
      case 'n_bar_high':
        fired = highN !== null && price >= highN * 0.998;
        break;
      case 'n_bar_low':
        fired = lowN !== null && price <= lowN * 1.002;
        break;
      case 'adx_trending':
        fired = adx !== null && adx > 25;
        break;
      case 'fib_level':
        fired = nearFib !== null;
        break;
...
export async function runScreener(
  ...
  strategyEval?: (
    sym: string,
    candles: Candle[]
  ) => { dir: 'long' | 'short'; score: number; reasons: string[] } | null,
  ...
  for (let batchStart = 0; batchStart < syms.length; batchStart += CONCURRENCY) {
    ...
    const batchResults = await Promise.allSettled(
      batch.map(async (sym, batchIdx) => {
        ...
        if (ind.bbWidth !== null) {
          symSamples.push(ind.bbWidth);
          if (symSamples.length > 50) symSamples.shift();
        }
...
export async function runMultiTFScan(
  ...
  const adapter = getAdapter(exchangeId);
  const delay = FETCH_DELAY[exchangeId];
  const baseResults = await runScreener(
    syms,
    tfs[0],
    [],
    onProgress,
      ...
      try {
        ...
        const trend: 'bull' | 'bear' | 'neutral' =
          ind.e9 && ind.e20 && ind.e50
            ? ind.e9 > ind.e20 && ind.e20 > ind.e50
              ? 'bull'
              : ind.e9 < ind.e20 && ind.e20 < ind.e50
                ? 'bear'
                : 'neutral'
  ...
  for (const r of results) {
    const cur: 'bull' | 'bear' | 'neutral' =
      r.ema9 && r.ema20 && r.ema50
        ? r.ema9 > r.ema20 && r.ema20 > r.ema50
          ? 'bull'
          : r.ema9 < r.ema20 && r.ema20 < r.ema50
            ? 'bear'
            : 'neutral'
