        ...
        if (s.allPairsMode) {
          ...
          try {
            ...
            set({
              screenerRunning: false,
              fetchingPairs: false,
              screenerError: `Failed to fetch pairs: ${String(e)}`,
            });
            return;
          }
        } else {
          ...
          if (!wl || !wl.syms.length) {
            set({ screenerError: 'No symbols in watchlist', screenerRunning: false });
            return;
          }
          syms = wl.syms;
        ...
        set((s) => {
          ...
          return {
            watchlists: s.watchlists.map((w) =>
              w.id === s.activeWatchlistId ? { ...w, syms: [...w.syms, sym.toUpperCase()] } : w
            ),
          };
        }),
