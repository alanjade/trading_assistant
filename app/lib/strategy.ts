import { STOP_LOSS, TP_PRESETS, POSITION_SIZING } from './tradingConstants';

// ── Type Guards ───────────────────────────────────────────────────────────────
export function isIndicatorOperand(op: Operand): op is { type: 'indicator'; id: IndicatorId } {
  return op.type === 'indicator';
}

export function isFixedOperand(op: Operand): op is { type: 'fixed'; value: number } {
  return op.type === 'fixed';
}

export function isPatternOperand(op: Operand): op is {
  type: 'pattern';
  id:
    | 'engulfing_bull'
    | 'engulfing_bear'
...
// ── Stop Type ─────────────────────────────────────────────────────────────────
export type StopType = 'fixed_pct' | 'atr_multiple' | 'swing_low' | 'bb_band';

// ── Type Guards for Stop Types ────────────────────────────────────────────────
export function isFixedPctStop(config: StopConfig): config is StopConfig & { type: 'fixed_pct' } {
  return config.type === 'fixed_pct';
}

export function isAtrMultipleStop(
  config: StopConfig
): config is StopConfig & { type: 'atr_multiple' } {
  return config.type === 'atr_multiple';
}

export function isSwingLowStop(config: StopConfig): config is StopConfig & { type: 'swing_low' } {
  return config.type === 'swing_low';
}

export function isBBandStop(config: StopConfig): config is StopConfig & { type: 'bb_band' } {
  return config.type === 'bb_band';
}

// ── Type Guards for Sizing Methods ────────────────────────────────────────────
export function isFixedUsdSizing(
  config: SizingConfig
): config is SizingConfig & { method: 'fixed_usd' } {
  return config.method === 'fixed_usd';
}

export function isFixedPctSizing(
  config: SizingConfig
): config is SizingConfig & { method: 'fixed_pct' } {
  return config.method === 'fixed_pct';
}

export function isRiskPctSizing(
  config: SizingConfig
): config is SizingConfig & { method: 'risk_pct' } {
  return config.method === 'risk_pct';
}

export function isKellySizing(config: SizingConfig): config is SizingConfig & { method: 'kelly' } {
  return config.method === 'kelly';
}
...
function makeRule(left: Operand, condition: Condition, right: Operand, lookback = 1): Rule {
  return { id: makeId(), left, condition, right, lookback };
}

const defaultStop: StopConfig = {
  type: 'atr_multiple',
  value: STOP_LOSS.ATR_MULTIPLIER,
  breakEvenAt: STOP_LOSS.BREAKEVEN_TRIGGER_R,
  trailAfter: STOP_LOSS.TRAIL_ACTIVATION_R,
  trailType: 'atr_multiple',
  trailValue: STOP_LOSS.TRAIL_ATR_MULTIPLIER,
};

const defaultTP: TakeProfitConfig = {
  targets: TP_PRESETS.DEFAULT_MULTI_TP.map((tp) => ({
    rrMultiple: tp.ratio,
    sizePercent: Math.round(tp.allocation * 100),
  })),
};

export const patchedDefaultSizing: SizingConfig = {
  method: 'risk_pct',
  value: POSITION_SIZING.DEFAULT_RISK_PERCENT * 100, // Convert 0.01 to 1
  kellyWinRate: 0.55,
  kellyAvgWinR: STOP_LOSS.TRAIL_ACTIVATION_R,
  kellyAvgLossR: 1.0,
  kellyFraction: POSITION_SIZING.KELLY_FRACTION,
  maxPerTrade: POSITION_SIZING.MAX_TRADE_SIZE,
  maxOpen: POSITION_SIZING.MAX_CONCURRENT_TRADES,
  maxDailyLoss: 0,
};
export const PRESET_STRATEGIES: Strategy[] = [
  {
    ...
    description:
      'Classic 3 EMA stack strategy. Long when EMA9 > EMA20 > EMA50, RSI in momentum zone. Short when stacked bearishly.',
    createdAt: 0,
    updatedAt: 0,
    enabled: true,
    longEntry: {
      logic: 'AND',
      rules: [
        makeRule({ type: 'indicator', id: 'ema9' }, 'greater_than', {
          type: 'indicator',
          id: 'ema20',
        }),
        makeRule({ type: 'indicator', id: 'ema20' }, 'greater_than', {
          type: 'indicator',
          id: 'ema50',
        }),
    ...
    shortEntry: {
      logic: 'AND',
      rules: [
        makeRule({ type: 'indicator', id: 'ema9' }, 'less_than', {
          type: 'indicator',
          id: 'ema20',
        }),
        makeRule({ type: 'indicator', id: 'ema20' }, 'less_than', {
          type: 'indicator',
          id: 'ema50',
        }),
  ...
  },
  {
    ...
    name: 'BB Bounce',
    description:
      'Mean reversion. Long when price touches lower BB and RSI is oversold. Short on upper BB touch with RSI overbought.',
    createdAt: 0,
    updatedAt: 0,
    enabled: true,
    longEntry: {
      logic: 'AND',
      rules: [
        makeRule({ type: 'indicator', id: 'price_close' }, 'less_equal', {
          type: 'indicator',
          id: 'bb_lower',
        }),
    ...
    shortEntry: {
      logic: 'AND',
      rules: [
        makeRule({ type: 'indicator', id: 'price_close' }, 'greater_equal', {
          type: 'indicator',
          id: 'bb_upper',
        }),
    ...
    takeProfit: {
      targets: [
        { rrMultiple: 1, sizePercent: 60 },
        { rrMultiple: 2, sizePercent: 40 },
      ],
    },
    sizing: patchedDefaultSizing,
  },
  {
    ...
    name: 'MACD Crossover',
    description:
      'Trend-following via MACD line crossing signal line. Filtered by EMA50 direction for trend alignment.',
    createdAt: 0,
    updatedAt: 0,
    enabled: true,
    longEntry: {
      logic: 'AND',
      rules: [
        makeRule({ type: 'indicator', id: 'macd_line' }, 'crosses_above', {
          type: 'indicator',
          id: 'macd_signal',
        }),
        makeRule({ type: 'indicator', id: 'price_close' }, 'greater_than', {
          type: 'indicator',
          id: 'ema50',
        }),
      ],
    ...
    shortEntry: {
      logic: 'AND',
      rules: [
        makeRule({ type: 'indicator', id: 'macd_line' }, 'crosses_below', {
          type: 'indicator',
          id: 'macd_signal',
        }),
        makeRule({ type: 'indicator', id: 'price_close' }, 'less_than', {
          type: 'indicator',
          id: 'ema50',
        }),
      ],
  ...
  },
  {
    ...
    description:
      'Counter-trend. Buys extreme oversold readings, sells extreme overbought. Best on ranging markets with ADX < 25.',
    createdAt: 0,
    updatedAt: 0,
    enabled: true,
    longEntry: {
  ...
  },
  {
    ...
    description:
      'Enters on SuperTrend direction flip. Confirmed by EMA stack alignment. Clean trend-following approach.',
    createdAt: 0,
    updatedAt: 0,
    enabled: true,
    longEntry: {
      logic: 'AND',
      rules: [
        makeRule({ type: 'indicator', id: 'supertrend_dir' }, 'crosses_above', {
          type: 'fixed',
          value: 0,
        }),
        makeRule({ type: 'indicator', id: 'ema9' }, 'greater_than', {
    ...
    shortEntry: {
      logic: 'AND',
      rules: [
        makeRule({ type: 'indicator', id: 'supertrend_dir' }, 'crosses_below', {
          type: 'fixed',
          value: 0,
        }),
        makeRule({ type: 'indicator', id: 'ema9' }, 'less_than', {
...
function resolveOperand(op: Operand, snap: IndicatorSnapshot): number | null {
  if (isFixedOperand(op)) {
    return op.value;
  }

  if (isPatternOperand(op)) {
    return snap.patterns.includes(op.id) ? 1 : 0;
  }

  if (isIndicatorOperand(op)) {
    const { id } = op;
    switch (id) {
      case 'price_close':
        return snap.price;
      ...
      case 'stoch_rsi_d':
        return snap.stochD;
      case 'macd_line':
        return snap.macdLine;
      case 'macd_signal':
        return snap.macdSig;
      case 'macd_hist':
        return snap.macdHist;
      case 'bb_upper':
        return snap.bbUpper;
      case 'bb_middle':
        return snap.bbMiddle;
      case 'bb_lower':
        return snap.bbLower;
      case 'bb_pct':
        return snap.bbPct;
...
function evalRule(rule: Rule, snap: IndicatorSnapshot): boolean {
  ...
  if (rule.condition === 'crosses_above') {
    ...
    const prevL = resolveOperand(rule.left, {
      ...snap.prev,
      prev: null,
      patterns: snap.prev.patterns ?? [],
    });
    const prevR = resolveOperand(rule.right, {
      ...snap.prev,
      prev: null,
      patterns: snap.prev.patterns ?? [],
    });
...
function evalCondition(
  ...
  if (cond.filters?.minADX && cond.filters.minADX > 0) {
    if (snap.adx === null || snap.adx < cond.filters.minADX) {
      return {
        pass: false,
        reasons: [`ADX ${snap.adx?.toFixed(1) ?? '—'} below min ${cond.filters.minADX}`],
      };
    }
  }
  ...
  const results = cond.rules.map((rule) => {
    ...
    const lDesc =
      rule.left.type === 'indicator'
        ? rule.left.id
        : rule.left.type === 'fixed'
          ? String(rule.left.value)
          : rule.left.id;
    const rDesc =
      rule.right.type === 'indicator'
        ? rule.right.id
        : rule.right.type === 'fixed'
          ? String(rule.right.value)
          : rule.right.id;
    return { pass, label: `${lDesc} ${rule.condition.replace(/_/g, ' ')} ${rDesc}` };
  });

  const pass = cond.logic === 'AND' ? results.every((r) => r.pass) : results.some((r) => r.pass);
...
export function calcStopPrice(
  ...
  switch (stop.type) {
    ...
    case 'bb_band': {
      return dir === 'long' ? (snap.bbLower ?? entry * 0.98) : (snap.bbUpper ?? entry * 1.02);
    }
    default:
...
export function buildSnapshot(storeState: {
  livePrice: number;
  ...
  const stDir = storeState.stBull.length
    ? storeState.stBull[storeState.stBull.length - 1]
      ? 1
      : -1
    : null;
  const prevStDir =
    storeState.stBull.length >= 2
      ? storeState.stBull[storeState.stBull.length - 2]
        ? 1
        : -1
      : null;
  const psarDir = storeState.psarBull.length
    ? storeState.psarBull[storeState.psarBull.length - 1]
      ? 1
      : -1
    : null;
  const prevPsarDir =
    storeState.psarBull.length >= 2
      ? storeState.psarBull[storeState.psarBull.length - 2]
        ? 1
        : -1
      : null;
  ...
  const snap: IndicatorSnapshot = {
    ...
    prev: prevBar
      ? {
          ...
          obv:
            storeState.obvVals.length >= 2
              ? storeState.obvVals[storeState.obvVals.length - 2]
              : null,
          ...
          cvdCum:
            storeState.cvdCumDeltas.length >= 2
              ? storeState.cvdCumDeltas[storeState.cvdCumDeltas.length - 2]
              : null,
          volume: prevBar.v,
