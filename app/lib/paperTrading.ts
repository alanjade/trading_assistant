...
import { STOP_LOSS } from './tradingConstants';
...
export function openPaperPosition(
  ...
  const tpLevels: PaperTPLevel[] = signal.targets.map((price, i) => ({
    price,
    sizePercent:
      strategy.takeProfit.targets[i]?.sizePercent ?? Math.round(100 / signal.targets.length),
    hit: false,
  }));
...
export function tickPosition(pos: PaperPosition, price: number, atr: number | null): TickResult {
  ...
  p.unrealised = isLong ? (price - p.entryPrice) * units : (p.entryPrice - price) * units;
  ...
  if (p.trailActive && atr !== null) {
    const trailDist = atr * STOP_LOSS.TRAIL_ATR_MULTIPLIER;
    const newTrail = isLong ? price - trailDist : price + trailDist;
    if (p.trailPrice === null) {
      p.trailPrice = newTrail;
    } else {
      p.trailPrice = isLong ? Math.max(p.trailPrice, newTrail) : Math.min(p.trailPrice, newTrail);
    }
