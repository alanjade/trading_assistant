...
const APIS: ApiDef[] = [
  {
    ...
    klines: (s, t, limit = 200, endTime?: number) =>
      `https://api.binance.com/api/v3/klines?symbol=${s}&interval=${t}&limit=${limit}${endTime ? `&endTime=${endTime}` : ''}`,
  ...
  },
  {
    ...
    ticker: (s) => `https://api.bybit.com/v5/market/tickers?category=spot&symbol=${s}`,
    parseKlines: (d) => {
  ...
  },
  {
    name: 'MEXC',
    klines: (s, t, limit = 200, endTime?: number) =>
      `https://api.mexc.com/api/v3/klines?symbol=${s}&interval=${t}&limit=${limit}${endTime ? `&endTime=${endTime}` : ''}`,
  ...
  },
  {
    name: 'Binance US',
    klines: (s, t, limit = 200, endTime?: number) =>
      `https://api.binance.us/api/v3/klines?symbol=${s}&interval=${t}&limit=${limit}${endTime ? `&endTime=${endTime}` : ''}`,
  ...
  },
  {
    ...
    parseKlines: (d) =>
      (d as number[][])
        .slice(-100)
        .map((k) => ({ o: +k[1], h: +k[2], l: +k[3], c: +k[4], v: 1000, t: +k[0] })),
    parseTicker: (d, s) => {
      const id = CG_MAP[s] || 'bitcoin';
...
export async function fetchKlines(
  ...
  for (let i = 0; i < APIS.length; i++) {
    ...
    try {
      ...
      if (candles.length === 0 || candles.some((c) => isNaN(c.c) || c.c <= 0))
